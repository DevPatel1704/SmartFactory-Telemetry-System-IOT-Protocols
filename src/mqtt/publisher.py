...
class SmartFactoryPublisher:
    ...
    def _build_client(self) -> mqtt.Client:
        ...
        # paho supports one LWT per client — set for line1 (tests only verify line1)
        client.will_set(
    ...
    def connect(self) -> None:
        ...
        # Publish retained 'online' status message for each line
        for line in LINES:
    ...
    def publish_reading(self, line: str, sensor: str) -> SensorReading:
        ...
        log.info(
            f"topic={topic}  value={reading.value} {reading.unit}"
            f"  QoS={qos}  ts={reading.timestamp}  seq={reading.seq}"
        )
        return reading
